...
def price_figure(df: pd.DataFrame, is_critical: bool) -> go.Figure:
    ...
    gradient = (
        [[0.0, "rgba(196,71,46,0.02)"], [1.0, "rgba(222,127,53,0.40)"]]
        if is_critical
        else [[0.0, "rgba(31,111,178,0.02)"], [1.0, "rgba(30,158,110,0.32)"]]
    )
    trace_kwargs = dict(
        x=df["Timestamp"],
        y=df["Price"],
    ...
    try:
        ...
        trace = go.Scatter(**trace_kwargs, fillcolor="rgba(196,71,46,.18)" if is_critical else "rgba(31,111,178,.15)")
    ...
    <div class="terminal-header">
      <div class="brand-lockup">
        ...
        <div>
          ...
          <div class="brand-subtitle">Portafolio algorítmico de José Bordas · Decision intelligence · Model observability</div>
        </div>
      </div>
      {status_badge}
    </div>
